      ...
      <div className="mx-auto w-full max-w-6xl space-y-8">
        ...
        <form onSubmit={handleSubmit} className="grid items-start gap-8 lg:grid-cols-12">
          ...
          <aside className="hidden rounded-lg bg-foreground p-6 text-background lg:sticky lg:top-8 lg:col-span-4 lg:block">
            ...
            <div className="space-y-4">{selectedModels.map((model, index) => <div key={model?.model_id || index} className="flex items-center gap-3"><span className={`flex h-9 w-9 items-center justify-center rounded-full bg-agent-${String.fromCharCode(97 + index)}-bg text-xs font-semibold text-agent-${String.fromCharCode(97 + index)}-fg`}>{String.fromCharCode(65 + index)}</span><div className="min-w-0"><p className="font-medium">{getAgentSoapName(`Agent ${String.fromCharCode(65 + index)}`, personas[index])}</p><p className="truncate text-xs text-background/60">{model?.display_name || model?.model_id || 'Selecting model'} · {model?.origin_region || 'OTHER'}</p></div></div>)}</div>
